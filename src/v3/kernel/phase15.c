// Phase 15: a compiled userland on the guest OS.
//
// Builds on the Phase 14 guest kernel (PMM, per-process address spaces, the
// timer scheduler, the syscall ABI, the FS read path, and file descriptors).
// The new kernel work supports a real compiled userland linked against a libc:
//
//   * executable loading: programs are loaded from a small header (magic, entry,
//     memSize) plus a flat text+data image, mapped across as many pages as the
//     image needs (text, data, and bss) -- not a single flat code page,
//   * argv passing: exec() takes (path, argv); the kernel copies the argument
//     strings and the argv[] array into the new process's user stack and enters
//     main(argc, argv),
//   * a unified file-descriptor table where each fd is the console, the
//     keyboard, an open file, or a pipe end, and
//   * pipe() / dup() with a blocking pipe (readers block until data or EOF),
//     so the shell can run `cmd | cmd`.
//
...
// and runs echo/cat/ls and pipelines -- all compiled C on the guest.
//
...
void capture_handlers() {
  asm("
    ...
    MOV R1, phase15_timer_handler
    STORE R1, timer_handler_addr
    MOV R1, phase15_pf_handler
...
int kmain() {
  asm("
    JMP phase15_handlers_done

  phase15_timer_handler:
    STORE R0, sctx_r0
    STORE R1, sctx_r1
    STORE R2, sctx_r2
    STORE R3, sctx_r3
  ...
  phase15_default_handler:
    CALL on_default_trap

  phase15_handlers_done:
  ");

  serial_write("phase15: boot\n");
  setup_traps();
  pmm_init();
  build_kernel_pt();
