...
**Status:** Phases 1–3 done — the virtual hardware ([`src/v2/hw/`](../src/v2/hw/):
CPU privilege levels, paging MMU, traps, port I/O), the kernel core
([`src/v2/kernel/`](../src/v2/kernel/): physical/virtual memory managers,
timer-driven scheduler, syscall dispatch, console driver), and the Unix **process
model** (`fork`/`exec`/`wait`/`exit`, an ELF-like loader, and reaping of zombies).
User-mode programs run in their own paging address spaces, syscall, get preempted,
and can fork children and exec other programs (demos: `node demo/v2-preempt.ts`,
`node demo/v2-fork-exec.ts`). The filesystem and userland are next. For the
