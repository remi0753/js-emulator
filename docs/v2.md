...
**Status:** Phases 1–4 done — the virtual hardware ([`src/v2/hw/`](../src/v2/hw/):
CPU privilege levels, paging MMU, traps, port I/O, **block disk**), the kernel core
([`src/v2/kernel/`](../src/v2/kernel/): physical/virtual memory managers,
timer-driven scheduler, syscall dispatch, console + disk drivers), the Unix
**process model** (`fork`/`exec`/`wait`/`exit`, an ELF-like loader, zombie reaping),
and **storage**: an xv6-style on-disk **filesystem** (superblock, inodes, block
...
disk. User-mode programs run in their own paging address spaces, syscall, get
preempted, fork/exec, and read & write files on a persistent `disk.img` (demos:
`node demo/v2-preempt.ts`, `node demo/v2-fork-exec.ts`, `node demo/v2-fs.ts`).
Userland (`init`, a shell, coreutils) is next. For the predecessor see [v1.md](v1.md).
