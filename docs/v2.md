...
**Status:** the **acceptance target is reached** — boot → shell → `ls` lists the
disk (`node demo/v2-shell.ts`). Phases 1–5 done:
...
`node demo/v2-fs.ts`. Remaining Phase 6 polish: a live keyboard with blocking
I/O, pipes, more syscalls, and copy-on-write `fork`. For the predecessor see
[v1.md](v1.md).
