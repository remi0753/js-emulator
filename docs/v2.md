...
**Status:** Phases 1–2 done — the virtual hardware ([`src/v2/hw/`](../src/v2/hw/):
CPU privilege levels, paging MMU, traps, port I/O) and the kernel core
([`src/v2/kernel/`](../src/v2/kernel/): physical/virtual memory managers,
timer-driven scheduler, syscall dispatch, console driver). User-mode programs now
run in their own paging address spaces, syscall, and get preempted (demo:
`node demo/v2-preempt.ts`). The process model, filesystem, and userland are next.
For the predecessor see [v1.md](v1.md).
